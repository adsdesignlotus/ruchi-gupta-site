...
export function EventExternalPhotoLinks({ slug }: { slug: string }) {
  ...
  const hasLinks = Boolean(drive || cloud);
  ...
  return (
    ...
    >
      ...
      {!hasLinks ? (
        <p className="mt-5 max-w-reading text-left text-sm italic text-text-secondary">
          Album links are not listed yet. Add a Google Drive or Cloudinary URL
          in Admin and save, or turn off &ldquo;Show on site&rdquo; until they are
          ready.
        </p>
      ) : (
